...
use yfi_config::{Config, LogFormat, StaticEnvironment};
...
mod tempdir {
    ...
    impl Temp {
        pub fn new(body: &str) -> Self {
            ...
            path.push(format!("yfi-config-{unique}-{:p}.toml", body.as_ptr()));
...
fn the_environment_layer_overrides_the_file() {
    ...
    assert_eq!(yfi_config::effective_severity(&config, Code::DuplicateKey), Severity::Allow);
}
...
fn flags_override_everything_else() {
    ...
    assert_eq!(yfi_config::effective_severity(&config, Code::AnchorShadowed), Severity::Allow);
...
fn every_diagnostic_code_is_configurable() {
    ...
    for code in Code::all() {
        ...
        assert_eq!(yfi_config::effective_severity(&config, *code), Severity::Warning);
        assert!(yfi_config::known_codes().contains(code.as_str()), "{code} must be listed");
    }
}
