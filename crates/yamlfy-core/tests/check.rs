...
use yamlfy_core::check::{check, check_with};
use yamlfy_core::intern::intern;
use yamlfy_core::link::link;
use yamlfy_syntax::{Code, Severity, SeverityMap};
...
fn the_merge_cycle_corpus_is_rejected_here() {
    // `yamlfy-syntax/tests/cycles.rs` asserts these four parse cleanly and
