...
pub(crate) fn every_holder(ctx: &Ctx) -> Vec<Place> {
    ...
    for file in ctx.project.files() {
        for position in 0..file.ast.nodes().len() {
            let node = yamlfy_syntax::NodeId(u32::try_from(position).expect("arena overflow"));
...
fn holds_members(ctx: &Ctx, file: yamlfy_syntax::FileId, node: yamlfy_syntax::NodeId) -> bool {
...
impl Run<'_> {
    ...
    fn gate_of(&self, file: yamlfy_syntax::FileId, key: yamlfy_syntax::NodeId, scope: ScopeId)
        -> FieldGate {
