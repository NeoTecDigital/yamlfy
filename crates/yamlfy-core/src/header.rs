...
use tracing::debug;
use yamlfy_syntax::{Ast, Code, Diagnostic, Diagnostics, NodeId, Span};
...
pub fn read(ast: &Ast, diagnostics: &mut Diagnostics) -> Option<Header> {
    ...
    let Some(entries) = ast.entries(root) else {
        // `--- !yamlfy/header` with no body is an empty scalar, not a mapping.
        ...
        diagnostics.push(Diagnostic::new(
            Code::BadHeaderValue,
            ast.node(root).span,
            "a `!yamlfy/header` document must be a mapping",
        ));
        return None;
    };
