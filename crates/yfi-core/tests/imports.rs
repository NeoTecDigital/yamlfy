...
use yfi_core::{intern, FileClass, TagKind};
use yfi_syntax::{Code, NodeId};
...
fn a_data_file_is_never_read_as_yamlfication() {
    // `nested-namespaces/net/edge.yaml` carries a `!yfi/header` document
    ...
    assert!(file.header.is_none(), "`!yfi/header` in base YAML is not a header");
...
fn yamlfication_tags_are_inert_in_a_data_file() {
    ...
    assert!(
        ...
        "`!node` and `!yfi/header` are inert in base YAML: {kinds:?}"
    );
